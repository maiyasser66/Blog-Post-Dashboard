...
<div class="container">
    ...
    <?php if (!isset($_SESSION['auth_user'])) { ?>
        <a href="signin.php" class="btn btn-primary mt-3">Create a New Post</a>
    <?php } else { ?>
        <a href="./posts/views/create-post.php" class="btn btn-primary mt-3">Create a New Post</a>
    ...
    <div class="row my-5">
        <?php while ($row = $result->fetch_assoc()) { ?>
            <div class="card text-center m-3" style="width: 18rem;">
                <img src="./posts/views/images/<?= !empty($row['image']) ? htmlspecialchars($row['image']) : 'default.jpg' ?>" class="card-img-top" alt="<?= htmlspecialchars($row['title']) ?>">
                <div class="card-body d-flex flex-column">
                    <h5 class="card-title"><?= $row['title'] ?></h5>
                    <p class="card-text text-start"><?= substr($row['content'], 0, 150) . '.....' ?></p>
                    <a href="./posts/views/post-details.php?id=<?= $row['id'] ?>" class="btn btn-primary mt-auto w-60 mx-auto">Post Details</a>
                </div>
                <div class="text-body-secondary pb-4">
                    <strong class="text-secondary"><?= htmlspecialchars($row['created_by']) ?> •<time><?= date("F j, Y, g:i a", strtotime($row['created_at'])) ?></time>
                    </strong>
                </div>
